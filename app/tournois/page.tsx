...
import { Calendar, MapPin, Search, ShieldAlert, Users } from "lucide-react";

export default async function TournoisPage({
  searchParams,
}: {
  searchParams: Promise<{ ville?: string; sport?: string }>;
}) {
  const session = await getSession();
  if (!session || session.role !== "ATHLETE") redirect("/connexion");

  const { ville, sport } = await searchParams;
  ...
  const mineurNonConsenti =
    isMineur(athlete.dateNaissance) && athlete.consentement?.codeValide !== true;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <form className="card flex flex-col gap-2 p-3 sm:flex-row" method="get">
        ...
        <select name="sport" defaultValue={sport ?? ""} className="input sm:w-44">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs" style={{ color: "var(--muted)" }}>
                    Organisé par {e.organisateur.nom}
                  </p>
                </div>
